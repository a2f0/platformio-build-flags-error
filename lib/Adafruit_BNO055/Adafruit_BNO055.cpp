...
imu::Vector<3> Adafruit_BNO055::getVectorRaw(adafruit_vector_type_t vector_type)
{
  imu::Vector<3> xyz;
  uint8_t buffer[6];
  memset (buffer, 0, 6);
...
{
  sensorWire.beginTransmission(_address);
  #if ARDUINO >= 100
    sensorWire.write((uint8_t)reg);
    sensorWire.write((uint8_t)value);
  #else
    sensorWire.send(reg);
    sensorWire.send(value);
  #endif
...
byte Adafruit_BNO055::read8(adafruit_bno055_reg_t reg )
{
  ...
  #endif
  sensorWire.endTransmission();
  sensorWire.requestFrom(_address, (byte)1);
  #if ARDUINO >= 100
    value = sensorWire.read();
  #else
    value = sensorWire.receive();
  #endif
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, byte * buffer, uint8_t len)
{
  ...
  #endif
  sensorWire.endTransmission();
  sensorWire.requestFrom(_address, (byte)len);

  for (uint8_t i = 0; i < len; i++)
  {
    #if ARDUINO >= 100
      buffer[i] = sensorWire.read();
    #else
      buffer[i] = sensorWire.receive();
    #endif
  }
